...
export async function POST(req: Request, res: NextResponse) {
    ...
    try {
        if (user && user.id) {
            ...
            const accessToken = await signJwtToken({ id: signedUpUser.id, role: signedUpUser.role })
            ...
            response.cookies.set({
                name: 'refreshToken',
                sameSite: 'strict',
                value: await signJwtRefreshToken({ id: signedUpUser.id, role: signedUpUser.role }),
                httpOnly: true,
                maxAge: 60 * 10,
                secure: false
            })
